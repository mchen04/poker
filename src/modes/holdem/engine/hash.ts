/**
 * Pure-JS synchronous SHA-256 + Web-Crypto random helpers.
 *
 * The PartyKit/workerd runtime exposes `crypto.getRandomValues` but NOT Node's
 * synchronous `createHash`/`randomBytes`. The deck commitment must be computed
 * synchronously inside `buildHand`, so we ship a small, dependency-free SHA-256
 * here instead of reaching for `crypto.subtle.digest` (async) or `node:crypto`.
 */
...
export function sha256Hex(message: string): string {
  ...
  // 64-bit length; high word is 0 for any practical message size here.
...
/** Cryptographically-random 32-byte seed as lowercase hex. */
